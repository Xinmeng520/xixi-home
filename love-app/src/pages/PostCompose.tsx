  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-warm-50 via-orange-50/20 to-warm-50 flex flex-col">
      ...
      <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-warm-100/60">
        ...
        <h1 className="text-sm font-medium text-warm-600 tracking-wide" style={{ fontFamily: "Georgia, 'Songti SC', serif" }}>记录我们的点点滴滴</h1>
        ...
        {files.length > 0 && (
          <div className="grid grid-cols-3 gap-3 mt-4">
            {files.map((file: File, i: number) => (
              <div key={i} className="aspect-square rounded-2xl overflow-hidden bg-warm-100 relative group border border-warm-200/50">
                <img src={URL.createObjectURL(file)} alt="" className="w-full h-full object-cover" />
                <button type="button" onClick={() => removeFile(i)} className="absolute top-2 right-2 w-6 h-6 rounded-full bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
